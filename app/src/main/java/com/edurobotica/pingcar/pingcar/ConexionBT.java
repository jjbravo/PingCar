...
//	public static final InputStream INPUT_Stream = null;

    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<< METODOS >>>>>>>>>>>>>>>>>>>>>>>>>>>    
    //----<<<<---->>>>----METODOS  NO  ALTERADOS----<<<<---->>>>----

    /**
     * Constructor. Prepara una nueva sesion para la conexion ConexionBT Smartphone-Dispositivo
    ...
    private synchronized void setState(int estado) {
        ...
        mHandler.obtainMessage(pingCarActivityMain.Mensaje_Estado_Cambiado, estado, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(pingCarActivityMain.Mensaje_Nombre_Dispositivo);
        Bundle bundle = new Bundle();
        bundle.putString(pingCarActivityMain.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(pingCarActivityMain.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(pingCarActivityMain.TOAST, "Error de conexión");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(pingCarActivityMain.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(pingCarActivityMain.TOAST, "Se perdio conexion");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        msg = mHandler.obtainMessage(pingCarActivityMain.MESSAGE_Desconectado);
        mHandler.sendMessage(msg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(pingCarActivityMain.Mensaje_Escrito, -1, -1, buffer).sendToTarget();
        ...
        public  void read() {
            ...
            while (true) { //Mantiene escuchando el InputStream mientras este conectado
                try {
                    ...
                    byte[] readBufX = buffer;  //Construye un String desde los bytes validos en el buffer
                    String readMessageX = new String(readBufX, 0, bytes);//Se envia el readNessagexxx en lugar del buffer pues ya se PARSEO               
                    mHandler.obtainMessage(pingCarActivityMain.Mensaje_Leido, bytes, -1, buffer).sendToTarget(); //readMessageX por buffer              
                    cadena=readMessageX;
